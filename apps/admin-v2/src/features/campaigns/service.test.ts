...
describe('campaign service', () => {
	it('authorizes regular creation with list access and optional template access', async () => {
		...
		await expect(createAuthorizedCampaign(createCommand, deps)).resolves.toEqual({ id: 22 });
		expect(deps.authorization.requirePermissions).toHaveBeenNthCalledWith(1, { campaign: ['create'], list: ['view'] });
		expect(deps.authorization.requirePermissions).toHaveBeenNthCalledWith(2, { template: ['view'] });
	...
	it('does not require template access when the default template is selected', async () => {
		const deps = dependencies();
		await createAuthorizedCampaign({ ...createCommand, templateId: null }, deps);
		expect(deps.authorization.requirePermissions).toHaveBeenCalledTimes(1);
	});

	it('rejects empty regular content, past schedules, missing or archived lists, and incompatible templates', async () => {
		await expect(createAuthorizedCampaign({ ...createCommand, body: ' ' }, dependencies())).rejects.toThrow('Enter campaign content');
		await expect(createAuthorizedCampaign({ ...createCommand, sendAt: '2020-01-01T00:00:00Z' }, dependencies())).rejects.toThrow('future');
		await expect(createAuthorizedCampaign({ ...createCommand, listIds: [99] }, dependencies())).rejects.toThrow('no longer exist');
		await expect(createAuthorizedCampaign({ ...createCommand, listIds: [13] }, dependencies())).rejects.toThrow('active');
		await expect(createAuthorizedCampaign({ ...createCommand, templateId: 2 }, dependencies())).rejects.toThrow('ordinary campaign email template');
		await expect(createAuthorizedCampaign({ ...createCommand, templateId: 4 }, dependencies())).rejects.toThrow('ordinary campaign email template');
	});

	it('creates a provider-owned opt-in message only for active double opt-in lists', async () => {
		const deps = dependencies();
		await createAuthorizedCampaign({ ...createCommand, type: 'optin', body: 'ignored', contentType: 'markdown' }, deps);
		expect(deps.manager.create).toHaveBeenCalledWith(expect.objectContaining({ type: 'optin', body: '', contentType: 'richtext' }));
		await expect(createAuthorizedCampaign({ ...createCommand, type: 'optin', listIds: [12] }, dependencies())).rejects.toThrow('double opt-in');
	});
	...
	it('re-fetches a draft, checks its version, and preserves provider-owned opt-in content', async () => {
		...
		await updateAuthorizedCampaign(updateCommand({ body: 'client does not own this' }), deps);
		expect(deps.manager.update).toHaveBeenCalledWith(expect.objectContaining({ body: optin.body, contentType: 'richtext' }));

		await expect(updateAuthorizedCampaign(updateCommand({ expectedUpdatedAt: '2026-08-25T12:00:00Z' }), dependencies())).rejects.toThrow('changed after');
		await expect(updateAuthorizedCampaign(updateCommand(), dependencies({ ...draft, status: 'scheduled' }))).rejects.toThrow('Only draft');
		await expect(updateAuthorizedCampaign(updateCommand(), dependencies({ ...draft, contentType: 'visual' }))).rejects.toThrow('visual editor');
	});
	...
	it('enforces the exact status graph and a future schedule on the server', async () => {
		...
		await transitionAuthorizedCampaign({ id: 21, expectedUpdatedAt: draft.updatedAt, transition: 'schedule' }, scheduleDeps);
		...
		await transitionAuthorizedCampaign({ id: 21, expectedUpdatedAt: draft.updatedAt, transition: 'unschedule' }, unscheduleDeps);
		expect(unscheduleDeps.manager.transition).toHaveBeenCalledWith(21, 'draft');

		await expect(transitionAuthorizedCampaign({ id: 21, expectedUpdatedAt: draft.updatedAt, transition: 'start' }, dependencies(scheduledDraft))).rejects.toThrow('Remove the scheduled time');
		await expect(transitionAuthorizedCampaign({ id: 21, expectedUpdatedAt: draft.updatedAt, transition: 'cancel' }, dependencies({ ...draft, status: 'scheduled' }))).rejects.toThrow('running or paused');
		...
		for (const [status, transition, target] of [
			...
			await transitionAuthorizedCampaign({ id: 21, expectedUpdatedAt: draft.updatedAt, transition }, deps);
		...
		await transitionAuthorizedCampaign({ id: 21, expectedUpdatedAt: draft.updatedAt, transition: 'resume' }, scheduledResume);
	...
	it('revalidates every selected draft before one bounded bulk delete', async () => {
		...
		await expect(deleteAuthorizedCampaigns({ campaigns: [{ id: 21, expectedUpdatedAt: draft.updatedAt }] }, unsafe)).rejects.toThrow('Only draft');
	...
	it('maps expected provider failures without leaking diagnostics', async () => {
		...
		await expect(createAuthorizedCampaign(createCommand, createDeps)).rejects.toThrow('rejected these campaign settings');

		const previewDeps = dependencies();
		previewDeps.manager.preview = vi.fn(async () => { throw new CampaignProviderFailure(422); });
		await expect(previewAuthorizedCampaign(21, previewDeps)).rejects.toThrow('could not render');
	});
});
