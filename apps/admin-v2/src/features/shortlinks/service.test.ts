...
describe('shortlink service boundary', () => {
	it('validates before authorization and provider access', async () => {
		...
		await expect(
			createAuthorizedShortlink({ ...validCreate, longUrl: 'javascript:alert(1)' }, setupResult.dependencies),
	...
	it('rejects extra command keys and invalid integer/date/tag/code values', async () => {
		for (const input of [
			...
			const setupResult = setup();
			await expect(createAuthorizedShortlink(input, setupResult.dependencies)).rejects.toThrow();
			expect(setupResult.enforcePermissions).not.toHaveBeenCalled();
		}
	});
	...
	it('normalizes typed create data, deduplicates tags, and uses create authority', async () => {
		const setupResult = setup();
		await expect(createAuthorizedShortlink(validCreate, setupResult.dependencies)).resolves.toEqual({
			ok: true,
			shortCode: 'press-kit',
		});
		expect(setupResult.enforcePermissions).toHaveBeenCalledWith({ shortlink: ['create'] });
	...
	it('never crosses the provider boundary after denied authorization', async () => {
		...
		await expect(listAuthorizedShortlinks(setupResult.dependencies)).rejects.toThrow('forbidden');
	...
	it('uses the exact permission for every operation', async () => {
		...
		await listAuthorizedShortlinks(setupResult.dependencies);
		await readAuthorizedShortlink('press-kit', setupResult.dependencies);
		await readAuthorizedEditableShortlink('press-kit', setupResult.dependencies);
		await readAuthorizedShortlinkOverview(setupResult.dependencies);
		await editAuthorizedShortlink({ ...validEdit, shortCode: 'press-kit' }, setupResult.dependencies);
		await resetAuthorizedShortlinkVisits('press-kit', setupResult.dependencies);
		await deleteAuthorizedShortlink('press-kit', setupResult.dependencies);
		expect(setupResult.enforcePermissions.mock.calls.map((call) => call[0])).toEqual([
			{ shortlink: ['view'] },
			{ shortlink: ['view'] },
			{ shortlink: ['edit'] },
			{ shortlink: ['view'] },
	...
	it('returns only editable configuration to an edit-only route', async () => {
		const setupResult = setup();
		await expect(readAuthorizedEditableShortlink('press-kit', setupResult.dependencies)).resolves.toEqual(editable);
		expect(setupResult.enforcePermissions).toHaveBeenCalledWith({ shortlink: ['edit'] });
	...
	it('validates a direct route capability before checking it', async () => {
		const setupResult = setup();
		await expect(requireAuthorizedShortlinkCapability('create', setupResult.dependencies)).resolves.toBe(true);
		expect(setupResult.enforcePermissions).toHaveBeenCalledWith({ shortlink: ['create'] });

		const invalid = setup();
		await expect(requireAuthorizedShortlinkCapability('publish', invalid.dependencies)).rejects.toThrow();
	...
	it('maps absence and conflict to stable public errors', async () => {
		...
		await expect(readAuthorizedShortlink('missing', missing.dependencies)).rejects.toThrow('Shortlink not found.');
		...
		await expect(createAuthorizedShortlink(validCreate, conflict.dependencies)).resolves.toEqual({
			ok: false,
			reason: 'conflict',
		});
	});
	...
	it('never deletes the seven short codes embedded in printed QR materials', async () => {
		for (const shortCode of ['signup', 'training', 'campus-training', 'sign-in', 'join', 'whatsapp', 'donate']) {
			const setupResult = setup();
			await expect(deleteAuthorizedShortlink(shortCode, setupResult.dependencies)).rejects.toMatchObject({
				name: 'PublicError',
				status: 409,
				message: 'This shortlink backs a printed QR code and cannot be deleted.',
			});
			expect(setupResult.enforcePermissions).toHaveBeenCalledWith({ shortlink: ['delete'] });
