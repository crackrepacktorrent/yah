...
async function expectSelectedEmailSection(page: Page, name: string): Promise<void> {
	...
	await expect(navigation.getByRole('link', { name, exact: true })).toHaveAttribute('data-selected');
}

async function expectSelectedPrimarySection(page: Page, name: string): Promise<void> {
...
test('primary and email navigation expose current state without overflowing narrow screens', async ({ page }, testInfo) => {
	...
	await page.setExtraHTTPHeaders({ 'x-forwarded-for': `127.0.0.${45 + testInfo.retry}` });
	await page.goto('/login');
	await page.getByLabel('Email').fill(ownerEmail);
	await page.getByLabel('Password').fill(ownerPassword);
	await page.getByRole('button', { name: 'Sign in' }).click();
...
test('email operators can safely update SMTP settings, test delivery, and inspect redacted logs', async ({ page, request }, testInfo) => {
	...
	expect(fixtureState.settings['bounce.actions'].hard).toEqual({ count: 1, action: 'unsubscribe' });
	expect(fixtureState.settings['bounce.mailboxes'][0]).toMatchObject({ enabled: true, password: '••••••' });
...
test('analytics is permission-discoverable, keyboard-selectable, semantic, and safely retryable', async ({ page, request }, testInfo) => {
	...
	await expect(page.getByRole('status').filter({ hasText: 'Updating analytics…' })).toBeVisible();
	await expect(pageviewStat).toHaveText('700');
...
test('campaigns preserve provider state, enforce stale versions, and use truthful status and opt-in workflows', async ({ page, request }, testInfo) => {
	...
	const campaignToolbar = page.getByRole('toolbar', { name: 'Campaign content formatting' });
