...
import { HttpError, surfaceError } from '~/server/http-errors';
import { ORG_SLUG } from '~/lib/constants';
import { nonEmptyStringSchema } from '~/lib/schemas';
import { parseInput } from '~/server/validation';
import * as v from 'valibot';

const invitationIdSchema = v.pipe(nonEmptyStringSchema, v.maxLength(255));
const completeAccountSchema = v.object({
	invitationId: invitationIdSchema,
	name: v.pipe(nonEmptyStringSchema, v.maxLength(100)),
...
export const getInvitationInfo = query(async (id: string): Promise<InvitationInfo | null> => {
	'use server';
	try {
		const invitationId = parseInput(invitationIdSchema, id);
...
export async function completeInvitationAccount(input: unknown): Promise<void> {
	'use server';
	try {
		const data = parseInput(completeAccountSchema, input);
		const headers = getWebRequest().headers;
		const session = await auth.api.getSession({ headers });
		if (!session?.user.emailVerified) throw new HttpError('Use the secure access link before setting a password.', 403);
		...
		if (!invitation || invitation.email.trim().toLowerCase() !== session.user.email.trim().toLowerCase()) {
			throw new HttpError('This invitation does not belong to the signed-in account.', 403);
		}
		...
		await auth.api.setPassword({ headers, body: { newPassword: data.password } });
	} catch (error) {
		surfaceError(error);
	}
}
...
/**
 * Accept only an invitation observed as live in this invocation. Historical
 * accepted records are never used to recreate removed memberships.
 */
export async function acceptInvitation(id: string): Promise<{ organizationId: string }> {
	...
	try {
		...
		if (!session?.user.emailVerified) throw new HttpError('Use the secure access link before accepting.', 403);

		const { internalAdapter } = await auth.$context;
		const accounts = await internalAdapter.findAccounts(session.user.id);
		if (!accounts.some((account) => account.providerId === 'credential' && account.password)) {
			throw new HttpError('Set a password before accepting this invitation.', 403);
		}
		...
		const invitation = result.rows[0];
		if (!invitation || invitation.email.trim().toLowerCase() !== session.user.email.trim().toLowerCase()) {
			throw new HttpError('This invitation is unavailable or does not belong to this account.', 403);
		}
