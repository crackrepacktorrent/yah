...
- [x] Port the remaining Listmonk capabilities with their owning feature slices rather than copying the current horizontal integration module wholesale.
  ...
  - [x] Do not migrate generic logs, settings/SMTP, import, media, or placeholder operations without a newly demonstrated product requirement and narrow ownership boundary.
...
The lazy content route is 215.96 kB raw / 67.37 kB gzip after moving the QR constructor into that already-lazy route and removing the extra waterfall. The lazy Better Auth 1.6.30 route is 30.42 kB / 11.50 kB. Neither enters the initial admin shell, and the client output contains no Solid 1 adapter imports. The current Solid 1 build ships the same Lexical modules as several chunks totalling more raw code, so a different editor engine is not justified by this migration. The editor and QR components currently prove lifecycle/compatibility, not the old editor toolbar or QR preset/logo/download feature parity; those belong to their feature slices.
...
- Rich-text editor: keep Lexical route-split at the measured 165.83 kB raw / 52.37 kB gzip; reconsider only if authoring needs change or another framework-neutral editor materially improves lifecycle, output compatibility, and total migration cost.
