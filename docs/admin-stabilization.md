...
- [x] Add semantic client/server import restrictions after feature boundaries exist; do not encode the current horizontal layout as permanent policy.
...
### Post-parity consolidation program

The post-parity review found that the vertical product architecture is sound, but migration work left repeated presentation and request mechanics. Consolidation is intentionally staged so a mechanical cleanup does not become a generic framework or obscure authorization/provider boundaries. This checklist is the durable progress record; each completed phase must add its measured line impact, validation, and any changed decision.

- [x] **Native UI grammar.** One shared source of truth now owns section navigation, form controls, two-column forms, metadata lists, badges, pagination, breadcrumbs, page headers, bulk actions, responsive dialog actions, and table loading surfaces. Twenty-three repeated page-header structures now use a narrow native `PageHeader`; dashboard tables/retry controls and confirmation controls reuse the existing table/button/error primitives. Domain-specific tones, actions, and selection policies remain explicit. The batch also fixes launch-facing root loading/404 treatment, full-width member and role forms, campaign-analytics client navigation, analytics period revalidation, long-title wrapping, and semantic delivery statistics. No component library, universal control wrapper, or generic data table was introduced.
- [x] **UI permission projection.** Both admins now consume one typed `can(session, resource, action)` helper from `@yah/admin-core`. It replaces the admin-v2 optional-chain permission expressions and controls discoverability only; every server operation continues to authorize independently through Better Auth.
- [x] **CSS ownership.** Rich-editor, toast, and compatibility-only rules now live beside their owners, and production `compat-editor-*` vocabulary is renamed `rich-text-editor-*`. The global entry is reduced to actual tokens/reset/shared loading. The email stylesheet was first stripped of shared declarations; further file splitting is optional organizational movement rather than prerequisite consolidation. A bulk Tailwind or CSS Modules rewrite remains rejected because it would relocate rather than eliminate complexity.
- [x] **Solid 2 query identity.** Main analytics intentionally uses `latest(snapshot)` because the retained snapshot labels its own period; its update announcement is visually hidden. Campaign analytics instead scopes each metric `Loading` boundary to its complete query. Subscriber search now client-navigates while retaining native GET fallback, and subscriber, bounce, and log result/selection/pagination slots use URL-query-scoped `Loading` boundaries so old rows cannot appear under a new search/page identity. Entity details, settings, dashboards, and mutations deliberately do not gain blanket `latest()`, `Reveal`, or `action()` usage.
- [x] **Production request boundary.** One 22-line typed helper now replaces 83 copies of the `getServerRequest` / runtime gate / dependency invocation / `surfaceError` sequence across fourteen explicit server modules. It captures the request before the first await, while every `'use server'` endpoint, name, argument, return type, dependency loader, and authorization call remains visible. This removed 406 production lines; four focused boundary tests bring the total phase change to net −327 lines. A CRUD/RPC factory remains rejected.
- [x] **Operation-scoped authorization.** Every production endpoint now constructs a small authorization context which privately captures its request headers, memoizes the canonical session once for that invocation, promise-caches each atomic Better Auth grant, and checks distinct grants in parallel. All thirteen application services are HTTP-free and receive only `requirePermissions()` / `getCurrentUserId()` authority. Better Auth remains authoritative; there is no global cache, DI container, or authorization from the client permission projection. This phase removes 59 production lines and adds a focused 83-line behavior suite, for +26 net including tests; combined with the request-boundary phase, production code is approximately 465 lines smaller and the total is approximately 301 lines smaller including tests.
- [x] **Command and parsing mechanics.** A narrow `createCommandTask` now owns only pending/error/`finally` mechanics in seven straightforward mutation flows; navigation, revalidation, success copy, conflicts, ambiguous delivery, preview state, and dialog lifecycles remain explicit. Eleven services share a public-input parser which exposes only schema-owned 400-level messages, while eight Listmonk/Shlink/Umami adapters share a separate private provider-response parser whose validation failures remain infrastructure errors. Four helper tests, 106 affected service tests, 127 adapter tests, type-check, lint, redirect-safety regressions, and diff validation pass. Solid 2 `action()` remains rejected as a generic loading-state replacement because these commands do not need Router-owned submission/revalidation semantics.
- [x] **Settings cohesion.** The former 529-line email-settings manager is now an 84-line coordinator over focused SMTP/general/performance/bounce/privacy protocols. One Listmonk document owner performs strict v6.2 validation, known-secret masking, unknown-field preservation, fresh GET → allowlisted patch → one full PUT, restart acknowledgement, and a process-wide URL-keyed write queue that releases after failure; concurrency is covered across independent manager instances. The large bounce draft replaces twenty-three scalar signal tuples with one Solid 2 store, while UUID-keyed mailboxes, smaller forms, SMTP collections, and explicit plain command projection remain unchanged where a store would not simplify ownership. This boundary adds 167 integration lines because previously implicit secret and protocol responsibilities are now independently testable; thirty-three focused tests, lint, and diff validation pass.
- [x] **Production browser-test ownership.** The 2,241-line deterministic upstream monolith is now a 49-line native router over eleven provider/feature modules, and the 2,091-line Playwright monolith is now twenty-four unchanged scenarios in seven feature specs over one shared worker-scoped guarded database fixture. The largest fixture file falls to 803 lines and the largest spec to 457 lines. Explicit state/reset boundaries, module imports, and a subscriber-search document-preservation regression increase the combined test source by 619 lines; literal/regex parity against the original fixture is exact, and discovery reports all twenty-four scenarios. Hono/MSW remain unnecessary because provider behavior—not dispatch syntax—is the valuable fixture contract.
- [x] **Final independent review and gates.** A separate read-only review found no unresolved Critical, High, Medium, or Low correctness, security, provider-fidelity, Solid 2 ownership, HTML/CSS, or maintainability findings. Its inexpensive cleanup removed the last duplicate page-header description declarations, shared the final repeated analytics dependency loader, and normalized the role-table markup. The first production browser run exposed one real interaction defect: a success toast could cover and indefinitely intercept an underlying action. Passive toast surfaces now ignore hit testing while their dismiss controls remain operable, and the access scenario explicitly proves the action works while the toast is still visible. Final repository type-check and lint pass without warnings; all 468 active admin-v2 unit tests pass with nine guarded tests skipped in the ordinary run; all six destructive Better Auth integration tests pass separately and remove their fixtures; fresh compatibility and production client/SSR builds pass; all fifteen compatibility scenarios pass; all twenty-four serialized built-production scenarios pass; and `git diff --check` is clean.

Cutover-only deletion remains deliberately separate: compatibility routes, the handwritten product-route migration gate, compatibility contracts/assets, obsolete icon dependencies, and the old admin are removed only after production cutover evidence and rollback sign-off. Until then they are safety scaffolding, not ordinary dead code.
...
- **Semantic runtime boundaries.** The Solid 2 Vite plugin already provides the correct transitive client/server guard: modules that touch request state, private environment values, Better Auth/database state, provider transport, or application services are marked with `import 'server-only'`. Feature `server.ts` files, session queries, middleware, and API handlers remain unmarked composition façades because Solid Start transforms their `'use server'` bodies. A temporary client import of an application service failed the production build at the exact boundary, while the complete server-function graph continued to build. Oxlint path restrictions, dependency-graph packages, and a custom directory allowlist were considered and rejected: they would only see direct specifiers or freeze today’s horizontal folder layout, while the native marker catches future transitive moves.
